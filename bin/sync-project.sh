...
# WHICH file owes the routing set is the whole point of the three-tier split, so ask only the
# file that owes it. CLAUDE.local.md carries the Baseline routing block (section 2d writes it).
# CLAUDE.md is regenerated by `mxcli init` and is a POINTER — per this toolkit's own authoring
# rule the per-tool entry points are pointers, never copies. Warning that a pointer "does not
# reference the Baseline routing set" told every freshly scaffolded project it was out of sync
# with itself: init-project.sh produced it, sync immediately complained about it, and --strict
# exited 3 on a project nobody had touched. Only when there is NO CLAUDE.local.md does CLAUDE.md
...
if [ -f "$PROJECT_DIR/CLAUDE.local.md" ]; then
  ...
  # by a mention of two Baseline-only skills surviving in CLAUDE.md (bootstrap's own "##
  # mxcli-project-toolkit Integration" section carries `skills/learned-mdl-preflight.md` /
  # `skills/query-the-model.md`). The anchor is deliberately NOT restricted to a `|`-table
  # row — a duplicate pasted as a bullet list would otherwise be invisible to this check — so
  # a CLAUDE.md that cites either skill for an unrelated reason (not a copied Baseline block)
  # can also trip it; the message below is phrased as "check whether" for that reason, not as
...
if [ -f "$_routing_owner" ] && ! grep -q "query-the-model" "$_routing_owner"; then
...
# ── Agent entry points ───────────────────────────────────────────────────────────────────
# Same story as the lint rules directly BELOW, for the same reason: `mxcli init` regenerates
...
#
# ASK BEFORE ACTING. wire-agents.sh calls `mxcli init`, which REGENERATES the entry points and
# then restores the preserved originals from .mxtk-backup/ — so every invocation leaves another
# timestamped backup set behind, whether or not anything needed doing. Calling it unconditionally
# made this script's own claim above ("no-op on a project that is already wired") false: four new
# files in .mxtk-backup/ per sync, forever, on a project where nothing had changed. `--check`
# writes nothing and exits non-zero only when wiring is actually missing, so it is the cheap
# question to ask first. Found 2026-08-19 by syncing one project four times and counting.
if [ "$WIRED" -eq 1 ] && [ -x "$SCRIPT_DIR/wire-agents.sh" ]; then
  if "$SCRIPT_DIR/wire-agents.sh" "$PROJECT_DIR" --check >/dev/null 2>&1; then
    :   # already wired — say nothing and touch nothing
  elif [ "$DRY_RUN" -eq 1 ]; then
    echo "Would run: bin/wire-agents.sh $PROJECT_DIR  (entry points are missing the wiring block)"
...
#
# THIS MUST RUN AFTER THE WIRING BLOCK ABOVE, not before it. Three toolkit rules
# (conv010_act_microflow_content, data_change_microflows, entity_business_key) also ship with
# mxcli, so the `mxcli init` inside wire-agents.sh overwrites them with its stock versions.
# Installed first, they were reverted seconds later by this same script, and the next sync
# dutifully "restored" them again — 3 artifacts reported updated on every run, forever, with
# the project never actually converging. Same fix as cdec1c4 made in init-project.sh, which
# never travelled to this file. Found 2026-08-19.
if [ "$WIRED" -eq 1 ]; then
