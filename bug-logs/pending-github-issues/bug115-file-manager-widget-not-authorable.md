...
**Source:** `bug-logs/mxcli-bugs.md`, `## BUG-115` (its 2026-09-05 addendum) — found 2026-09-05 (mxcli v0.19.0-nightly.c836f01, Mendix 11.14.0). This is BUG-115's upstream draft: no draft existed for the entry before this one, and the second project's finding (the round trip drops an existing widget with `check` clean) was logged on its own branch as a separate bug before being folded into BUG-115 at merge. Both projects' evidence belongs in one issue: the original entry's Studio-Pro-bundled `FileUploader` finding (`widget init` cannot extract it — there is no `.mpk`) is the reason no user-side workaround exists.
...
## The same gap, seen from the pluggable side (second project, mxcli v0.20.0, Mendix 11.12.1)

The pluggable `com.mendix.widget.web.fileuploader.FileUploader` parses but fails at exec with
`no definition for widget com.mendix.widget.web.fileuploader.FileUploader`: it is
Studio-Pro-bundled rather than shipped as an `.mpk`, so `mxcli widget init` cannot extract it
(it lands in that command's `skipped (built-in)` count). Unlike a marketplace widget there is no
user-side workaround — the definition has to come from mxcli or from Studio Pro.
