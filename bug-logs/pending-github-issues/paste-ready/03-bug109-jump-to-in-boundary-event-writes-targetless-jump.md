`JUMP TO <task>` inside an interrupting boundary-event body writes a Jump activity named after its target with no `Target` property — native `mx check` fails CE0495 + CE6680; the same statement in a user-task outcome is fine
