A page-level `actionbutton` (outside any dataview) with `Action: show_page Page(Param: $Var)` silently drops the argument and rebinds every target-page parameter to `$currentObject` — CE1571 at build
