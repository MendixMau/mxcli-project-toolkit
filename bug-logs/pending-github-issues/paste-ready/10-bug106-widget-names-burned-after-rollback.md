Widget names claimed by a failed exec stay "taken" after the `.mpr` is restored from a snapshot — retrying the corrected script fails with duplicate-name errors for names that no longer exist
