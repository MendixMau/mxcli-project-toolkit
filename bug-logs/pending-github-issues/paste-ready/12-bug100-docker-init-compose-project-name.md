`mxcli docker init` writes compose files into `.docker/` without `COMPOSE_PROJECT_NAME`, so every mxcli project on a machine resolves to Compose project `docker` and shares containers and one Postgres volume
