`raise error;` used directly in a microflow's main flow (not inside an error handler) always fails native mxbuild with `CE0710` — a genuine flow-graph codegen defect
