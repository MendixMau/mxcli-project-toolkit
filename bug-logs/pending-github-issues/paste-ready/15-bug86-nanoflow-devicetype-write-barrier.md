MDL044 write barrier (`currentDeviceType()` validation) only runs for `CREATE MICROFLOW`, not `CREATE NANOFLOW` — the identical invalid expression passes `check`, is written, and fails the build with `CE0117`
