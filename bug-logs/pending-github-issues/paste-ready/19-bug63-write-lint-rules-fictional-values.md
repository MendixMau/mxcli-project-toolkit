The bundled `write-lint-rules` skill documents `action_type`/`source_type` API values that do not exist — rules written from the guide's own examples silently match zero violations
