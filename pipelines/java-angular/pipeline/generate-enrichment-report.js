...
const totalEntities   = brds.reduce((s, b) => s + b.domainEntities.length, 0);
const totalMicroflows = brds.reduce((s, b) => s + b.microflows.length, 0);
const totalPages      = brds.reduce((s, b) => s + b.pages.length, 0);
const totalUseCases   = brds.reduce((s, b) => s + b.useCases.length, 0);
const reviewedUseCases = brds.reduce((s, b) => s + b.useCases.filter(u => u.reviewStatus === 'reviewed').length, 0);
const totalOpenQuestions = brds.reduce((s, b) => s + (b.openQuestions || []).length, 0);
const totalHiddenRules = brds.reduce((s, b) => s + b.microflows.reduce((n, m) => n + (m.hiddenRules || []).length, 0), 0);
...
const moduleSections = brds.map(b => {
  const entityRows = b.domainEntities.map(e => `
    <tr>
  ...
  const hiddenRules = b.microflows.flatMap(m => (m.hiddenRules || []).map(hr => ({ ...hr, microflow: m.name })));
  ...
  const pageByName = new Map(b.pages.map(p => [p.name, p]));
  ...
  <section class="module-section">
    ...
    <div class="stat-row">
      <div class="stat"><div class="val">${b.domainEntities.length}</div><div class="lbl">Entities</div></div>
      <div class="stat"><div class="val">${b.microflows.length}</div><div class="lbl">Functions</div></div>
      <div class="stat"><div class="val">${b.pages.length}</div><div class="lbl">Pages</div></div>
...
<head>
<meta charset="UTF-8">
<title>Phase 4 Enrichment Summary</title>
<style>
...
<div class="hero">
  <h1>Inventory Management System — Phase 4 Enrichment Summary</h1>
  <p>An inventory management system used to track item transactions: adding new items,
  selling items, inserting (restocking) items, editing/deleting items, and viewing a daily
  profit/sold/inserted summary. Sourced from README.md + screenshots (no external business
  docs were available for this pilot).</p>
  <div class="tech-tags">
    <span>Angular 11</span><span>Angular Material</span><span>TypeScript</span>
    <span>Spring Boot 2.4.5</span><span>Spring Data JPA</span><span>HATEOAS</span><span>MySQL 8.0</span>
  </div>
</div>
