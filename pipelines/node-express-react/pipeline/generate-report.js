...
// ── All use cases aggregated across BRD modules ───────────────────────────────
const allUseCases = [];
for (const [modName, brd] of Object.entries(brdByModule)) {
  for (const uc of (brd.useCases || [])) {
    allUseCases.push({ ...uc, _module: modName });
  }
}

const ucTableRows = allUseCases.length ? allUseCases.map(uc => {
  const flow  = Array.isArray(uc.mainFlow) ? uc.mainFlow.join(' → ') : '—';
  const oq    = Array.isArray(uc.openQuestions) && uc.openQuestions.length
                ? uc.openQuestions.map(q => typeof q === 'string' ? q : q.question).join('; ')
                : '—';
  const status = uc.reviewStatus === 'reviewed' ? 'reviewed' : (uc.status || 'code-inferred');
  const sCol   = status === 'reviewed' || status === 'doc-confirmed'
    ? '#dcfce7;color:#15803d'
    : status === 'doc-conflict'
      ? '#fee2e2;color:#b91c1c'
      : '#eff6ff;color:#1d4ed8';
  return `<tr onclick="showModule('${esc(uc._module)}')" style="cursor:pointer">
    <td class="mono">${esc(uc.id||'—')}</td>
    <td class="mono">${esc(uc._module)}</td>
    <td>${esc(uc.title||uc.screen||'—')}</td>
    <td style="font-size:12px;color:#475569;max-width:340px">${esc(flow)}</td>
    <td style="font-size:12px;color:#475569;max-width:260px">${esc(oq)}</td>
    <td><span style="background:${sCol};padding:2px 8px;border-radius:12px;font-size:11px;font-weight:600">${esc(status)}</span></td>
  </tr>`;
}).join('\n') : '<tr><td colspan="6" style="color:#94a3b8;font-style:italic;padding:12px">No use cases found in BRD files.</td></tr>';

// ── Quality narrative ─────────────────────────────────────────────────────────
const highConf   = moduleData.filter(m => m.confidence === 'high').length;
const medConf    = moduleData.filter(m => m.confidence === 'medium').length;
const lowConf    = moduleData.filter(m => m.confidence === 'low').length;
const totalMods  = moduleData.length;
const gapHeavy   = moduleData.filter(m => m.gapCount > 5).map(m => m.name);
const noEntities = moduleData.filter(m => m.counts.entities === 0 && m.counts.logics > 0).map(m => m.name);
const noLogics   = moduleData.filter(m => m.counts.logics === 0 && m.counts.screens > 0).map(m => m.name);
const ucCoverage = allUseCases.length > 0
  ? Math.round((allUseCases.filter(u => u.reviewStatus === 'reviewed').length / allUseCases.length) * 100)
  : 0;

const qualityCards = [
  { label: 'High-confidence modules', val: highConf, color: '#22c55e', note: 'No gaps — safe to start MDL' },
  { label: 'Medium-confidence modules', val: medConf, color: '#f59e0b', note: '1–3 gaps — minor review needed' },
  { label: 'Low-confidence modules', val: lowConf, color: '#ef4444', note: '>3 gaps — human enrichment required' },
  { label: 'Use cases extracted', val: allUseCases.length, color: '#3b82f6', note: `${ucCoverage}% reviewed` },
].map(c => `<div class="stat-card"><div class="val" style="color:${c.color}">${c.val}</div><div class="lbl">${c.label}</div><div style="font-size:11px;color:#94a3b8;margin-top:4px">${c.note}</div></div>`).join('');

const gapHeavyHtml = gapHeavy.length
  ? gapHeavy.map(n => `<span onclick="showModule('${esc(n)}')" style="cursor:pointer;background:#fee2e2;color:#b91c1c;padding:2px 8px;border-radius:12px;font-size:12px;margin:2px;display:inline-block">${esc(n)}</span>`).join(' ')
  : '<span style="color:#22c55e;font-size:13px">None — all modules have ≤5 gaps</span>';

const noEntitiesHtml = noEntities.length
  ? noEntities.map(n => `<span style="background:#fef9c3;color:#854d0e;padding:2px 8px;border-radius:12px;font-size:12px;margin:2px;display:inline-block">${esc(n)}</span>`).join(' ')
  : '<span style="color:#22c55e;font-size:13px">None</span>';
...
<head>
<meta charset="UTF-8">
<title>Node/Express/React Extraction Report</title>
<style>
...
<div class="sidebar">
  <div class="sidebar-header">
    <h1>Node/Express/React Extraction Report</h1>
  ...
  <div class="nav-item" onclick="showPage('usecases')">Use Cases</div>
  <div class="nav-item" onclick="showPage('quality')">Quality Summary</div>
...
<div class="main">
  ...
  <div class="page active" id="page-dashboard">
    ...
    <table style="margin-bottom:28px">
      ...
      <tbody>
        ...
        <tr><td>Express Route Handler</td><td>${logics.filter(l=>l.logicKind==='action'||l.logicKind==='process').length}</td><td>Microflow / Workflow</td></tr>
  ...
  <div class="page" id="page-usecases">
    <h2 style="margin-bottom:6px">All Use Cases</h2>
    <p style="color:#64748b;font-size:13px;margin-bottom:20px">${allUseCases.length} use cases extracted across ${Object.keys(brdByModule).length} BRD modules. Click a row to inspect the module. Status <em>code-inferred</em> = scaffold only; needs Phase 5 enrichment.</p>
    <table>
      <thead><tr><th>ID</th><th>Module</th><th>Title</th><th>Main Flow</th><th>Open Questions</th><th>Status</th></tr></thead>
  ...
  <div class="page" id="page-quality">
    <h2 style="margin-bottom:20px">Quality Summary</h2>
    <div class="stat-grid" style="margin-bottom:28px">${qualityCards}</div>

    <div class="section-block" style="margin-bottom:16px">
      <h3 style="margin-bottom:10px">Readiness Interpretation</h3>
      <table>
        <thead><tr><th>Signal</th><th>What it means</th><th>Next step</th></tr></thead>
        <tbody>
          <tr><td>High confidence (${highConf}/${totalMods})</td><td>No cross-reference gaps — entity, logic, and screen links resolved cleanly</td><td>Ready for MDL scripting (Phase 6)</td></tr>
          <tr><td>Medium confidence (${medConf}/${totalMods})</td><td>1–3 gaps — minor FK or screen-logic links missing</td><td>Review BRD gaps section, clarify in Phase 5</td></tr>
          <tr><td>Low confidence (${lowConf}/${totalMods})</td><td>>3 gaps — extractor could not link constructs confidently</td><td>Human BRD enrichment required before MDL</td></tr>
          <tr><td>Use case coverage</td><td>${allUseCases.length} use cases, ${ucCoverage}% reviewed</td><td>Review frontend BRD — fill actors, pre/postconditions, open questions</td></tr>
        </tbody>
      </table>
    </div>

    <div class="section-block" style="margin-bottom:16px">
      <h3 style="margin-bottom:10px">Gap-heavy modules (>5 gaps) — prioritise for BRD enrichment</h3>
      <div style="padding:6px 0">${gapHeavyHtml}</div>
    </div>

    <div class="section-block">
      <h3 style="margin-bottom:10px">Modules with logic but no entities — may need struct/NPE mapping</h3>
      <div style="padding:6px 0">${noEntitiesHtml}</div>
    </div>
  </div>
