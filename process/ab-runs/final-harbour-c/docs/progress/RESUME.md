...
`<TOOLKIT_ROOT>/skills/close-the-loop.md`.
...
1. Run `<TOOLKIT_ROOT>/bin/gate-check.sh .` — it reports the current stage from the files on disk.
...
2. **If it reports Stage P:** run the kickoff interview against `intake.md`. Question 1 is the
   ...
   `<TOOLKIT_ROOT>/skills/interview-protocol.md` §3. Record each answer in `PROJECT.md` as
...
| Working out what stage anything is in | `<TOOLKIT_ROOT>/skills/conversion-runbook.md` |
...
| Rewriting this file at the end of a session | `<TOOLKIT_ROOT>/skills/close-the-loop.md` |
