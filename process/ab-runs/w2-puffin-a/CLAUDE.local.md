...
`/home/user/mxcli-project-toolkit/bin/sync-project.sh`. Paths below are the toolkit conventions — **confirm each exists
at build time and correct any that differ**; a path that doesn't resolve is a wiring failure the
`build-ready` gate will catch.

| Key | Path | Notes |
|---|---|---|
| Toolkit root | `/home/user/mxcli-project-toolkit` | The process authority |
...
1. `git -C /home/user/mxcli-project-toolkit pull --ff-only`
2. `/home/user/mxcli-project-toolkit/bin/status.sh <project-root> --brief` — three lines: where the project is,
...
3. If it says an update is available, **you (the agent) handle it — never ask the user to type
   a command.** Run `/home/user/mxcli-project-toolkit/bin/gate-check.sh <project-root> --ack-protocol --verbose`,
   ...
   `/home/user/mxcli-project-toolkit/bin/gate-check.sh <project-root> --ack-protocol --approved-in-chat` — that
...
This project uses the shared toolkit at `/home/user/mxcli-project-toolkit`. For ANY pipeline work
(analysis, BRDs, architecture, design, build plan, build, test, cutover):

1. **Read `/home/user/mxcli-project-toolkit/skills/conversion-runbook.md` FIRST — every session.** It is the
...
4. **Before calling a stage done:** `/home/user/mxcli-project-toolkit/bin/gate-check.sh <project-root> <stage>`.


## Baseline routing (always-on)

GENERATED between the markers from `/home/user/mxcli-project-toolkit/bin/lib/skill-routing.tsv`. Refresh after a
toolkit pull with `/home/user/mxcli-project-toolkit/bin/sync-project.sh <project-root>`. Do not hand-edit inside the
...
| Always relevant for | Reference this (under `/home/user/mxcli-project-toolkit/`) |
|---|---|
