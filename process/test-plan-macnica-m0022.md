...
### Scenario H1: Register a new payer (Save / Save Draft)

| Step | Action | Expected result | Risk / Note |
|---|---|---|---|
| H1-1 | Navigate to PayerRegistration_Overview | Gallery loads; 0 records if fresh DB | — |
| H1-2 | Click New registration request | Payer_OrgChoice opens; ApplicantDept pre-filled from Employee record | Fails if no Employee (P3) |
...
| H1-6 | Click Next | Payer_Confirm_Selection opens; shows company + org summary | P7f2 script needed |
| H1-7 | Click Create order | PayerDetail_NewEdit opens | P7g script needed |
| H1-8 | Fill required fields (currency, etc.) | Form accepts input | Fields TBD in script |
| H1-9 | Click Save | ACT_PayerDetail_SaveDraft runs; PayerDetail created in DB; status → '01'; navigate to Overview | Wrapper MF needed (see §6) |
| H1-10 | Overview shows new record with status '01' | Row appears in gallery | Status shows raw '01' until CommonCategory seeded (P5) |
| H1-11 | Click Detail on the new record | PayerDetail_View opens | P7h script needed |
...
| H2-2 | Click WF request instead of Save | ACT_Payer_Submit runs: SaveDraft → validation → duplicate check → STUB_ACT_WFApplication_Submit |
