...
| 10 | enum `DECISION` **without** an Empty outcome  | `'Probe.StatusEnum.Draft'` → mxcli **rejects**. `'StatusEnum.Draft'` → mxcli **rejects**. `'Draft'` → mxcli ✓, exec ✓, round-trip ✓, **`mx check` cannot load the project: `StorageLoadException`** | **CORRUPTING — an enum decision cannot be written at all** | **BUG-108.** The validator is inverted: the only accepted form corrupts, both correct forms are refused. The Empty-outcome question could not even be reached. Recovery is `DROP WORKFLOW` (mxcli can still read the corrupt model) — verified back to 0 errors. |
