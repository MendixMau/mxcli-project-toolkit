...
#
# Full rationale: the toolkit's skills/close-the-loop.md. The toolkit root is resolved below
# from project context, never assumed to sit at any particular absolute path.
#
# USAGE
...
# Where the shared toolkit lives. Resolved from the PROJECT this script was invoked in, in
# priority order, because a script installed into many projects on many machines cannot know
# any one absolute path — and the previous version hardcoded one, which is why its routing
# row pointed somewhere that existed on exactly one laptop.
#   1. $MXTK_ROOT if the caller set it
#   2. the "Toolkit root" row of this project's CLAUDE.local.md Wiring block (the documented
#      single source of truth for project paths)
#   3. the directory this script was installed from, if it still looks like a toolkit clone
...
MXTK_ROOT="${MXTK_ROOT:-<toolkit root not resolved — set MXTK_ROOT>}"
...
if [ -s "$LEDGER" ]; then
  ...
  echo "- (none recorded — either nothing happened, or nothing is appending to .claude/.pending-writes)"
fi
echo
