...
**Purpose:** Assemble a new mxcli-powered project's `CLAUDE.md` from this toolkit's standardized Baseline routing plus that project's own, genuinely project-specific facts — so a new project starts with the same discipline `a Java/Angular analysis project` had to retrofit, instead of discovering the gap after microflows already exist with no annotations.
...
**Source:** Written after finding `a Java/Angular analysis project`'s `CLAUDE.md` never routed to `learned-microflow-patterns.md` despite the toolkit having the rule — a gap that existed because nothing generated that project's routing table against a standard.
...
| **Source platform / stack** | Decides which situational skills apply (`source-os11.md` vs `source-node-express-react.md` vs `pipelines/java-angular/` vs none, if greenfield) | Ask the user, or run `source-triage.md` Step 1 if a migration is already underway |
...
## Step 2: Route the Baseline Table to Where It Belongs — CLAUDE.local.md if It Exists, CLAUDE.md Otherwise

**Check for `CLAUDE.local.md` first.** `bin/init-project.sh` already writes it, carrying the Baseline
routing table (and the situational one) rendered from `bin/lib/skill-routing.tsv`, refreshed on
every `bin/sync-project.sh` run. Pasting the same table into `CLAUDE.md` on top of that does not
add coverage — it makes every session load the identical block twice, and the copy in `CLAUDE.md`
has no sync to keep it current, so it starts drifting from the day it's written.

- **`CLAUDE.local.md` exists (the normal case for any project scaffolded with `init-project.sh`):**
  do **not** copy the Baseline table into `CLAUDE.md`. Instead add a short pointer block, 3–5
  lines, and nothing else:

  ```
  ## Toolkit wiring

  Session ritual, Baseline routing, and situational routing live in `CLAUDE.local.md`
  (generated by `bin/init-project.sh`, refreshed by `bin/sync-project.sh`). Read it every
  session. Do not copy its tables here.
  ```

- **`CLAUDE.local.md` does not exist** (a non-Claude harness with no CLAUDE.local.md convention,
  or a project bootstrapped before `init-project.sh` ran): copy this toolkit's `README.md`
  "Baseline routing" table as-is into `CLAUDE.md`. Do not trim rows because "this project
  probably won't need X" — Baseline means it applies regardless of task, and the point of this
  step is removing that judgment call, not re-making it per project.

---

## Step 3: Select Situational Rows for This Project's Actual Stack

**Same split as Step 2.** If `CLAUDE.local.md` exists, its situational table is the one to extend
(sync keeps the Baseline rows current; situational rows for this project's stack still need adding
by hand there) — `CLAUDE.md` gets no situational copy either, the Step 2 pointer already covers it.
If `CLAUDE.local.md` does not exist, add the rows below to `CLAUDE.md` directly.
...
- **Copying a skill's own table into `CLAUDE.md`** — e.g. pasting `learned-mdl-preflight.md`'s write-mode table in whole. A copy goes stale the day the skill changes; point at the skill instead. Same reasoning as routing the Baseline table itself: when `CLAUDE.local.md` exists, `CLAUDE.md` gets a pointer, never a second copy of anything the toolkit already owns.
...
- **Treating this as a one-time-per-project skill.** Re-run Step 1–2 whenever the toolkit's Baseline table changes (a `git pull` brought in a new baseline-worthy skill) — an existing project's `CLAUDE.md` goes stale the same way `a Java/Angular analysis project`'s did.
