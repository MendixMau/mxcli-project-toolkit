...
8. Navigation wire point for every page (Step 8), and the prototype route it is built against when a prototype exists
...
### Every page names its prototype route

When the project has a clickable prototype (`design-artifacts.md` Step 3), the page list in the plan carries one more column: the route of the screen the page is built against, copied from the BRD's `pages[].route` (`brd-generation.md`).

| Page | Module | Wire point | Prototype route |
|------|--------|------------|-----------------|
| `Order_Overview` | OrderRegistration | Main nav > Orders | `#/order-list` |
| `Order_NewEdit` | OrderRegistration | opened from `Order_Overview` | `#/order-edit` |

WHY. The build checks find a page's wireframe by name, and page names and wireframe names drift apart as soon as the target model is renamed (`check-page-shell.sh` explains the project where every page missed). `PAGE-MAP.tsv` is the fix, and this column is its content: each row becomes one map line, written once here instead of reconstructed page by page during the build.

```
# design/wireframes/PAGE-MAP.tsv, generated from the table above
Order_Overview	../prototype.html#/order-list
Order_NewEdit	../prototype.html#/order-edit
```

A page with no route is either a page no use case reaches (check it against `brd-validation.md` check 8 before planning a script for it) or a page with no prototype screen, which `ui-preflight-pages.md` Step 1 stops on. Decide which now; an empty cell is not a decision.
