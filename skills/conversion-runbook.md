...
0. **The app you are changing already exists as a Mendix `.mpr` and stays live** → **Change an existing app** — `existing-app-change.md`. Rules 1–3 assume the target does not exist yet; applied to a live Mendix app they send it to Migration, i.e. to migrating itself (field run, 2026-08-20). Record the mode as `Change an existing app`; `gate-check.sh` parses it.
...
| **A live Mendix app you are adding to or altering** | **Change an existing app** | P (light), 0 (slice + blast radius), 1–6 over the slice; 7 N/A | `existing-app-change.md` is the owning skill. The knowledge base is the model itself (Path D, `mxcli`), not an extractor. The regression net comes first and is not optional. Requirements are `mxcli brain` slice entries with forward anchors; `brain plan` is the coverage ledger. |
