...
└────────────────────────────────────────────────────────────────────────────────┘
                                    ▲ specified by
┌─ LAYER 0 · SPEC (skills) ──────────────────────────────────────────────────────┐
│  journey-proof.md         five rungs, seven mutants, verdict discipline         │
│  journey-examples.md      worked journeys + spec-vs-runner gaps                 │
...
### Two known holes in that discipline — check yours

Both are present in the reference implementation at the time of writing. **VERIFIED.**

1. **The crash net's absence is recorded as `SKIPPED` with rc 0** and does not increment the fault
   count. On a project where `monkey.js` was simply never installed, the harness reports a clean
   pass over an instrument that does not exist.
2. **The CLEAN verdict text is unconditional.** It names journey and crash-net coverage in its
   scope sentence even after `--skip-journeys --skip-monkey`, or when the instrument is absent.
   That is the exact false green the harness exists to retire.

The fix for both is small and the right shape is: **generate the scope sentence from the run
summary**, naming only rungs whose verdict was PASS, and append an explicit
`NOT MEASURED: <rung> (<reason>)` line. An operator-requested skip may still exit 0 — they asked
for it — but **the verdict text must never claim what the flag suppressed.**
