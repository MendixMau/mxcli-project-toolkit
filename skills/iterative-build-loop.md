...
- [ ] **If this module contains a `CREATE WORKFLOW`: read `workflow-structure-rules.md` before writing it** — the whole file, in this session, not a citation of it. Then carry its §12 counts and the build plan's construct row into the module checklist: every construct the Stage-3 diagram asks for is either written in MDL here, or is a numbered **hand-add in Studio Pro** row that must be ticked before the module is marked done. A hand-add row silently dropped is invisible to `mxcli check`, to `mx check`, and to the module's own tests — the model is green and the construct is absent. Copy the hand-add rows verbatim from `brd-to-build-plan.md` → "A module carrying a workflow gets a construct row"; do not re-derive them.
