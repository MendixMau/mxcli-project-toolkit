...
| 8 | **Is the page shell right — nav chrome, breadcrumb, page title/H1, top padding — not just the content region?** Against the wireframe's shell when one exists; unaided otherwise. The scaffold every full page owes is `design-spacing.md` §3 — a page without it is a finding regardless of yardstick. | no page title where the wireframe has an H1; an un-skinned default nav bar; content starting flush under the breadcrumb; a sidebar design shipped as a top-bar app with nobody deciding that; **the navigation profile's own items — bare text labels where the design calls for icons, wrong order, no grouping.** The menu is chrome on every screen and belongs to no page, so a page-by-page review never reaches it: judge it once, explicitly, on the first screen you open. Menu-item icons are scriptable in MDL (`ICON Module.Collection.Name`) — see `learned-sidebar-collapse-icons.md` before concluding otherwise |
