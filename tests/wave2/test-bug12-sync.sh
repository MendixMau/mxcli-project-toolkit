...
echo "== T12: a hand-written CLAUDE.md duplicating the Baseline table triggers the new warn; a pointer-only CLAUDE.md does not =="
# bootstrap-project.md's old Step 2 told a merge to paste the Baseline table into CLAUDE.md even
# when CLAUDE.local.md (written by init-project.sh) already carries it — every session then loads
# the identical block twice. sync-project.sh now detects the duplicate and warns, but never edits
# CLAUDE.md (report-only, same as the retired-ledger-row and 2d-table-refusal checks above).
P="$(mkproj t12)"
cat > "$P/CLAUDE.md" <<'EOF'
# Project
...
| Always relevant for | Reference this |
|---|---|
| Writing ANY MDL script | `~/Mendix/mxcli-project-toolkit/skills/learned-mdl-preflight.md` |
| Any question before asking the user or writing anything | `~/Mendix/mxcli-project-toolkit/skills/query-the-model.md` |
EOF
B="$(fingerprint "$P")"
OUT="$("$SYNC" "$P" 2>&1)"
A="$(fingerprint "$P")"
[ "$B" = "$A" ] && ok "duplicated CLAUDE.md: sync wrote nothing (report-only)" \
                || bad "duplicated CLAUDE.md: sync WROTE" "$(printf 'before:\n%s\nafter:\n%s' "$B" "$A")"
case "$OUT" in
  *"CLAUDE.md cites Baseline-only skills"*"duplicat"*) ok "duplicated CLAUDE.md: the warn fires" ;;
  *) bad "duplicated CLAUDE.md: warn missing" "$OUT" ;;
esac

# Replace with the Step 2 pointer block (skills/bootstrap-project.md) — no |-table row, so the
# duplication anchor must not match it.
cat > "$P/CLAUDE.md" <<'EOF'
# Project
...
OUT="$("$SYNC" "$P" 2>&1)"
A="$(fingerprint "$P")"
[ "$B" = "$A" ] && ok "pointer-only CLAUDE.md: sync wrote nothing" \
                || bad "pointer-only CLAUDE.md: sync WROTE" "$(printf 'before:\n%s\nafter:\n%s' "$B" "$A")"
case "$OUT" in
  *"duplicat"*) bad "pointer-only CLAUDE.md: the warn fired on a pointer, not a duplicate" "$OUT" ;;
