...
# --- The 2026-09-08 field loss: a KEPT file that this suite already claimed to cover. --------
#
# The four cases below all exercise "sync must not clobber work", which the header of this file
# has asserted since it was written. They are here because the assertion was true and the INPUT
# could not exercise it: P4 above carries no MXTK-TRIAGE-STUB marker at all, so it is kept for
# the trivial reason (no marker -> not a scaffold) and never reaches the placeholder logic where
# the defect lived. Hand-written test input encoded the same assumption the code made.
#
# What really happens: the shipped template puts `<!--` and the marker on ONE line, but an agent
# that fills the file in naturally REFORMATS that header — wrapping it, adding a "filled in" note,
# and (observed) renaming the marker to MXTK-TRIAGE-STUB-FILLED to say so. Nothing tells them not
# to; the file says to keep the header. Both edits broke the predicate, and a signed-off Stage 0
# was overwritten on a real project — 73 lines of decisions, recovered only from git.
#
# So these cases are built by REFORMATTING the real template rather than by inventing a header.

# (a) Reformatted two-line header, file filled in -> kept. This is the exact loss.
P4b="$TMP/p4b"; clone "$PRISTINE" "$P4b"
{
  echo '<!--'
  echo '     MXTK-TRIAGE-STUB — generated by bin/init-project.sh from bin/lib/triage-template.sh.'
  echo '     This marker plus any remaining {{PLACEHOLDER}} is how bin/sync-project.sh knows the file'
  echo '     is still an untouched scaffold. -->'
  echo ''
  echo '# Source Triage: Acme Orders'
  echo ''
  echo '## Sign-off'
  echo 'Confirmed by: M. Visser on 2026-09-08'
} > "$P4b/triage.md"
BEFORE=$(cat "$P4b/triage.md")
OUT=$("$SYNC" "$P4b" 2>&1)
hasnt "a filled file with a REFORMATTED two-line header is not refreshed" "$OUT" "Refreshed: triage.md"
check "  and not a byte of it changes" "$(cat "$P4b/triage.md")" "$BEFORE"

# (b) The -FILLED marker is an explicit completion signal and outranks any placeholder.
#     It was swallowed by a substring grep for 'MXTK-TRIAGE-STUB' until 2026-09-08, so the
#     suffix that exists to say "done" said nothing.
P4c="$TMP/p4c"; clone "$PRISTINE" "$P4c"
{
  echo '<!-- MXTK-TRIAGE-STUB-FILLED — generated by init; header kept per its own instruction.'
  echo '     Explains the {{PLACEHOLDER}} mechanism and is retained deliberately. -->'
  echo '# Source Triage: Acme Orders'
} > "$P4c/triage.md"
BEFORE=$(cat "$P4c/triage.md")
OUT=$("$SYNC" "$P4c" 2>&1)
hasnt "-FILLED outranks a live placeholder in the header" "$OUT" "Refreshed: triage.md"
check "  and not a byte of it changes" "$(cat "$P4c/triage.md")" "$BEFORE"

# (c) No regression: a genuinely unfilled scaffold with a TWO-line header is still refreshable.
#     Without this, the fix for (a) would be "never refresh anything", which is not a fix.
P4d="$TMP/p4d"; clone "$PRISTINE" "$P4d"
{
  echo '<!--'
  echo '     MXTK-TRIAGE-STUB — explains the {{PLACEHOLDER}} mechanism. -->'
  echo '# Source Triage: {{APPLICATION_NAME}}'
} > "$P4d/triage.md"
OUT=$("$SYNC" "$P4d" 2>&1)
has "a genuinely unfilled two-line scaffold is still refreshed" "$OUT" "Refreshed: triage.md"

# (d) The guard that does not depend on getting a regex right: git says it is modified.
#     Three fixes have now narrowed the placeholder inference; this one sidesteps it. A file
#     with uncommitted changes has been worked on, whatever its markers say.
P4e="$TMP/p4e"; clone "$PRISTINE" "$P4e"
if command -v git >/dev/null 2>&1; then
  ( cd "$P4e" && git init -q . && git add -A >/dev/null 2>&1 \
    && git -c user.email=t@t -c user.name=t commit -qm base >/dev/null 2>&1 ) || true
  # Deliberately a PRISTINE-LOOKING scaffold: only git knows it was touched.
  printf 'edited\n<!-- MXTK-TRIAGE-STUB -->\n{{APPLICATION_NAME}}\n' > "$P4e/triage.md"
  BEFORE=$(cat "$P4e/triage.md")
  OUT=$("$SYNC" "$P4e" 2>&1)
  has "an uncommitted triage.md is kept on git's word alone" "$OUT" "uncommitted local changes"
  check "  and not a byte of it changes" "$(cat "$P4e/triage.md")" "$BEFORE"
else
  ok "git absent — skipping the git-modified guard case (not a failure)"
fi
